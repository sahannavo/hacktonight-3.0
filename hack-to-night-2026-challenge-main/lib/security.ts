...
if (!SESSION_SECRET) {
  console.warn(
    '[security] WARNING: SESSION_SECRET is not set. Using a random fallback. ' +
    'Set SESSION_SECRET in your environment for persistent session validation.'
  );
}

function getSecret(): string {
  return SESSION_SECRET || 'dev-fallback-secret-change-me';
}

// ─── Session Token (HMAC-signed, cryptographically secure) ─────────
...
export function generateSessionToken(userId: number, role: string): string {
  ...
  const signature = createHmac("sha256", getSecret())
    .update(`${nonce}.${payloadB64}`)
    .digest("hex");
  return `${nonce}.${payloadB64}.${signature}`;
}
